...
def fft_l1_loss(x, y, criterion):
    ...
    x_fft_complex = torch.fft.fft2(x, dim=(-2, -1), norm="ortho")
    y_fft_complex = torch.fft.fft2(y, dim=(-2, -1), norm="ortho")

    x_fft = torch.stack((x_fft_complex.real, x_fft_complex.imag), dim=-1)
    y_fft = torch.stack((y_fft_complex.real, y_fft_complex.imag), dim=-1)

    return criterion(x_fft, y_fft)
